...
public class PokemonService {
    ...
    public PokemonService() throws Exception{
        ...
        for(int i=0; i<N ; i++){
           ...
           Pokemon pkm = new Pokemon(pokemond_id 
                   ...
                   , pokemon_base_spAttack
                   , pokemon_base_spDefense);
   ...
   //ตัวอย่างการค้นหาข้อมูลใน list โดยผลลัพธ์อาจจะมีมากกว่า 1 ตัว เงื่อนไขการค้นหาในช่วงข้อมูล
    public ArrayList<Pokemon> searchByAttack(double min_SpAttack, double max_SpAttack){
        ArrayList<Pokemon> results = new ArrayList<Pokemon>();
        for(int i=0; i<pokemon_list.size(); i++){
            Pokemon pkm = pokemon_list.get(i);
            if(pkm.getSpAttack() >=min_SpAttack && pkm.getAttack() <= max_SpAttack){
                results.add(pkm);
            }
        }
        return results;
    ...
    public void insertPokemon(Integer id, String name_english, Double hp, Double attack, Double defense, Double spAttack  ){
        ...
        if(search_pokemon==null){
            //สร้างออบเจคโปเกมอน
            Pokemon pkm = new Pokemon(id, name_english, hp, attack, defense,  spAttack);
            pokemon_list.add(pkm);
...
//+updatePokemon(id : Integer, name_english : String, hp : Double,attack : Double,defense : Double,speed : Double)
    public boolean updatePokemon(Integer id, String name_english, Double attack, Double defense, Double spAttack, Double spDefense){
        //ค้นหาว่ามี pokemon id นี้แล้วหรือยัง?
        Pokemon search_pokemon = searchById(id);
        ...
        if(search_pokemon!=null){
            ...
            search_pokemon.setSpAttack(spAttack);
            search_pokemon.setSpDefense(spDefense);
    ...
    ArrayList<Pokemon> searchByNameEnglish(Double search_attack) {
